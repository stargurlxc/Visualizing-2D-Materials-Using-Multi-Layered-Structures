# -*- coding: utf-8 -*-
"""
Created on Fri Jul 10 13:19:00 2026

@author: Mariah Ellis

The purpose of this script was to replicate the graphene contrast plot used in "Making Graphene Visible" by P. Blake et. al.
https://pubs.aip.org/apl/article/91/6/063124/1023191/Making-graphene-visible

SiO2 thickness and the graphene index are both functions of wavelength.
...
L = np.arange(400,750,10) # wavelength in microns
...
siO2_thickness = np.arange(0, 350, 10)
graphene_thickness = .34
parallel_rk = 0
incident_angle = np.deg2rad(0)

air_Rs12 = np.zeros((len(siO2_thickness), len(L)))
air_Rp12 = np.zeros((len(siO2_thickness), len(L)))
air_Ts12 = np.zeros((len(siO2_thickness), len(L)))
air_Tp12 = np.zeros((len(siO2_thickness), len(L)))


graphene_Rs12 = np.zeros((len(siO2_thickness), len(L)))
graphene_Rp12 = np.zeros((len(siO2_thickness), len(L)))
graphene_Ts12 = np.zeros((len(siO2_thickness), len(L)))
graphene_Tp12 = np.zeros((len(siO2_thickness), len(L)))

#calculates P1 for air and graphene
for n in range(2):
    for jj, thickness in enumerate(siO2_thickness):     #looping over siO2 thickness (duh) 
        for ii, lam in enumerate(L):                    #looping over wavelength
            ...
            #graphene                                                       2.6 + 1.3j
            if n == 1:
                [P1, rk1, perpen_rk1, parallel_rk, k0] = Propagation_Matrix(2, nk1[ii], graphene_thickness, lam, parallel_rk, incident_angle)
            else:
                [P1, rk1, perpen_rk1, parallel_rk, k0] = Propagation_Matrix(2, nk2[ii], graphene_thickness, lam, parallel_rk, incident_angle)
            #siO2
            [P2, rk2, perpen_rk2, parallel_rk, k0] = Propagation_Matrix(3, nk3[ii], thickness, lam, parallel_rk,  incident_angle)
            #si100
...
plt.pcolormesh(siO2_thickness, L, contrast.transpose(), cmap='YlGnBu', shading='gouraud')
plt.colorbar(label='Contrast')
plt.title('Graphene Contrast')
plt.ylabel('λ (nm)')
