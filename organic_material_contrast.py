# -*- coding: utf-8 -*-
"""
Created on Mon Jul 20 16:10:17 2026

@author: Mariah Ellis 


The purpose of this script was to produce a plot of the contrast of an organic material 
with a reflective index of 1.7, using the thickness of the materials as a function of 
wavelength
...
#thicknesses in nm
siO2_thickness = 90 
organic_thickness = np.arange(0, 90, 1) 

parallel_rk = 0
incident_angle = np.deg2rad(0)


#empty arrays to store values in l8r
air_Rs12 = np.zeros((len(organic_thickness), len(L)))
air_Rp12 = np.zeros((len(organic_thickness), len(L)))
air_Ts12 = np.zeros((len(organic_thickness), len(L)))
air_Tp12 = np.zeros((len(organic_thickness), len(L)))

organic_Rs12 = np.zeros((len(organic_thickness), len(L)))
organic_Rp12 = np.zeros((len(organic_thickness), len(L)))
organic_Ts12 = np.zeros((len(organic_thickness), len(L)))
organic_Tp12 = np.zeros((len(organic_thickness), len(L)))

#for calculating two different P1 matrices: one for air index and organic material index
for n in range(2):
    for jj, thickness in enumerate(organic_thickness):  #looping over the material thickness
        for ii, lam in enumerate(L):                    #looping over wavelength
            ...
            if n == 1:
                ...
                [P1, rk1, perpen_rk1, parallel_rk, k0] = Propagation_Matrix(2, 1.7, thickness, lam, parallel_rk, incident_angle)
            #siO2
            ...
            #layer 2 is graphene
                ...
                organic_Rs12[jj, ii] = Ss[0,0]
                organic_Ts12[jj, ii] = Ss[0,1]
            
                organic_Rp12[jj, ii] = Sp[0,0]
                organic_Tp12[jj, ii] = Sp[0,1]
                
contrast = (air_Rs12 - organic_Rs12 ) / air_Rs12


plot_contrast(organic_thickness, L, contrast, 'bp')
plt.xlabel("Thickness (nm)")
plt.ylabel("λ (nm)")
plt.title("Organic Material Contrast using 90 nm SiO₂")
plt.show()